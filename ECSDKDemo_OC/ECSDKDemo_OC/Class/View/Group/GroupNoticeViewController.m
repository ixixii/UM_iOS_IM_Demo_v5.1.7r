...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (msg.messageType == ECGroupMessageType_Dissmiss) {
        ...
        cell.portraitImg.image = [[DemoGlobalClass sharedInstance]getOtherImageWithPhone:message.member];
        cell.contentLabel.text = [NSString stringWithFormat:@"\"%@\"%@%@\"%@\"的邀请加入",[self getMemberName:message.member andNickName:message.nickName],message.confirm==2?@"同意":@"拒绝",name,groupName];

...
-(ECSession *)noticeConvertToSession:(ECGroupNoticeMessage*)msg {
    ...
    if (msg.messageType == ECGroupMessageType_Dissmiss) {
        ...
        session.text = [NSString stringWithFormat:@"\"%@\"%@%@\"%@\"的邀请加入",[self getMemberName:message.member andNickName:message.nickName],message.confirm==2?@"同意":@"拒绝",name,groupName];
